...
def serialize_product(p: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Build product_images list for frontend carousel
    raw_images = p.get("product_images") or []
    images_out = [
        {
            "id":            img.get("id"),
        ...
        for img in raw_images if isinstance(img, dict) and img.get("image_url")
    ]

    # Compute total stock from variants (source of truth)
    total_stock = int(p.get("total_stock") or 0)
    if variants_out:
        total_stock = sum(v["stock"] for v in variants_out)
    ...
    return {
        ...
        "seller":         p.get("seller") if isinstance(p.get("seller"), dict) else {"first_name": seller_name, "last_name": ""},
        "variants":       variants_out,
        "product_variants": variants_out,
        "product_images": images_out,
