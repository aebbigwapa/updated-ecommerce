...
class _AdminSellersScreenState extends State<AdminSellersScreen> {
  ...
  Future<void> _load() async {
    try {
      ...
      if (mounted) {
        setState(() {
          _sellers = List<Map<String, dynamic>>.from(data).where((a) => a['role'] == 'seller').toList();
          _isLoading = false;
        });
      }
    } catch (_) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: const Text('Sellers', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700)),
        leading: IconButton(icon: const Icon(Icons.arrow_back, color: Colors.white), onPressed: () => Navigator.pop(context)),
      ),
      body: _isLoading
          ? const Center(child: CircularProgressIndicator(color: AppTheme.primaryLight))
          : _sellers.isEmpty
              ? const Center(child: Text('No sellers yet.', style: TextStyle(color: AppTheme.textLight)))
              : RefreshIndicator(
                  onRefresh: _load,
                  child: ListView.builder(
                    padding: const EdgeInsets.all(AppTheme.md),
                    itemCount: _sellers.length,
                    itemBuilder: (_, i) => _sellerTile(_sellers[i]),
                  ),
                ),
    );
  }
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Expanded(child: Text(s['full_name'] ?? '—', style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14))),
        ...
        Text(s['email'] ?? '—', style: const TextStyle(fontSize: 12, color: AppTheme.textLight)),
        if ((s['store_name'] ?? '').toString().isNotEmpty) ...[
          const SizedBox(height: 4),
          Text('🏪 ${s['store_name']}', style: const TextStyle(fontSize: 12, color: AppTheme.textDark)),
        ],
        if (status == 'pending') ...[
          const SizedBox(height: 10),
          Row(children: [
            Expanded(child: OutlinedButton(
              onPressed: () => _updateStatus(s['id'], 'approved'),
